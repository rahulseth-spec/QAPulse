...
const Users: React.FC<UsersProps> = ({ user, projects, token, onSelfUpdated, onUnauthorized }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetchWithAuth('/api/users', token, {}, onUnauthorized);
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          if (!cancelled) setError(typeof data?.error === 'string' ? data.error : 'Unable to load users');
          return;
        }
  ...
  const togglePerm = (area: PermissionArea, field: 'view' | 'edit') =>
    setPerms(prev => ({ ...prev, [area]: { ...prev[area], [field]: !prev[area][field] } }));
  ...
  const invite = async () => {
    ...
    setInviteResetUrl('');
    try {
      const res = await fetchWithAuth('/api/users/invite', token, {
        method: 'POST',
        body: JSON.stringify({ email, name, role: inviteRole, projects: inviteProjectIds }),
      }, onUnauthorized);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setInviteError(typeof data?.error === 'string' ? data.error : 'Unable to invite user');
        return;
      }
      const created = data?.user as ApiUser | undefined;
      if (!created?.id) { setInviteError('Unable to invite user'); return; }
      setItems(prev => [created, ...prev.filter(u => u.id !== created.id)]);
      setSelectedId(created.id);
      if (typeof data?.resetUrl === 'string' && data.resetUrl) setInviteResetUrl(data.resetUrl);
    } catch {
  ...
  const save = async () => {
    ...
    setSaveOk('');
    try {
      const res = await fetchWithAuth(`/api/users/${encodeURIComponent(selected.id)}`, token, {
        method: 'PATCH',
        body: JSON.stringify({ role, projects: projectIds, permissions: perms }),
      }, onUnauthorized);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setSaveError(typeof data?.error === 'string' ? data.error : 'Unable to save user');
        return;
      }
      const saved = data?.user as ApiUser | undefined;
      if (!saved?.id) { setSaveError('Unable to save user'); return; }
      setItems(prev => prev.map(u => u.id === saved.id ? saved : u));
      if (saved.id === user.id) onSelfUpdated(saved);
      setSaveOk('Saved');
    } catch {
      ...
      <div className="bg-white border border-slate-200 rounded-[20px] shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr]">
          <div className="border-b lg:border-b-0 lg:border-r border-slate-200">
            <div className="p-5">
              {canManage && (
                <button type="button" onClick={openInvite} className="h-10 w-full mb-4 rounded-xl bg-[#073D44] text-white font-semibold text-[13px] hover:bg-[#073D44]/90 transition-colors">
                  Invite User
                </button>
              )}
              {loading ? (
                  ...
                  {items.map(u => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl border transition-colors ${
                        u.id === selectedId ? 'bg-[#073D44] text-white border-[#073D44]' : 'bg-white text-slate-900 border-slate-200 hover:border-[#407B7E]/40'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {(() => {
                          ...
                          return <div className={`text-[10px] font-extrabold uppercase tracking-widest px-2 py-1 rounded-lg ${cls}`}>{label}</div>;
                        })()}
                      </div>
                    </button>
                  ))}
            ...
            {!selected ? (
              ...
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <button type="button" onClick={save} disabled={saving} className="h-10 px-4 rounded-xl bg-[#073D44] text-white font-semibold text-[13px] hover:bg-[#073D44]/90 disabled:opacity-60 transition-colors">
                    {saving ? 'Saving…' : 'Save'}
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
                    ...
                    <div className="mt-3">
                      <select
                        value={role}
                        onChange={e => { const r = normalizeRole(e.target.value); setRole(r); setPerms(JSON.parse(JSON.stringify(DEFAULT_PERMISSIONS_BY_ROLE[r]))); }}
                    ...
                    <div className="mt-3 text-[12px] text-slate-500 font-semibold">Manager has full access like super admin.</div>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {projects.map(p => (
                        <label key={p.id} className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 bg-white">
                          <input type="checkbox" checked={projectIds.includes(p.id)} onChange={() => toggleProject(p.id)} className="h-4 w-4 accent-[#073D44]" />
                    ...
                    <div className="mt-3 text-[12px] text-slate-500 font-semibold">Users can view reports for assigned projects.</div>
                  </div>
                </div>
                    ...
                    {areas.map(a => (
                      <div key={a.key} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border border-slate-200 rounded-2xl p-4">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-[12px] text-slate-500 font-semibold">{a.supportsEdit ? 'View + Edit supported' : 'View only'}</div>
                        </div>
                        <div className="flex items-center gap-3">
                          <label className="flex items-center gap-2 text-[12px] font-semibold text-slate-700">
                            <input type="checkbox" checked={perms[a.key].view} onChange={() => togglePerm(a.key, 'view')} className="h-4 w-4 accent-[#073D44]" />
                            View
                          </label>
                          <label className={`flex items-center gap-2 text-[12px] font-semibold ${a.supportsEdit ? 'text-slate-700' : 'text-slate-400'}`}>
                            <input type="checkbox" checked={perms[a.key].edit} disabled={!a.supportsEdit} onChange={() => togglePerm(a.key, 'edit')} className="h-4 w-4 accent-[#073D44]" />
                            Edit
                          </label>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-[20px] shadow-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 bg-[#CFE8E8] border-b border-[#073D44]/15 flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={() => setInviteOpen(false)} className="h-9 px-3 rounded-xl border border-slate-200 bg-white text-slate-700 font-semibold text-[13px] hover:bg-slate-50 transition-colors">
                Close
              </button>
            </div>
            <div className="p-6 space-y-5">
              {inviteError && <div className="text-[12px] font-semibold text-rose-700">{inviteError}</div>}
              {inviteResetUrl && <div className="text-[12px] font-semibold text-emerald-700 break-all">Reset link: {inviteResetUrl}</div>}

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="text-[12px] font-extrabold uppercase tracking-widest text-slate-700">Email</div>
                  <input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-800 focus:outline-none focus:ring-4 focus:ring-[#407B7E]/20" placeholder="user@company.com" />
                </div>
                <div>
                  <div className="text-[12px] font-extrabold uppercase tracking-widest text-slate-700">Name</div>
                  <input value={inviteName} onChange={e => setInviteName(e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-800 focus:outline-none focus:ring-4 focus:ring-[#407B7E]/20" placeholder="User" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="text-[12px] font-extrabold uppercase tracking-widest text-slate-700">Role</div>
                  <select value={inviteRole} onChange={e => setInviteRole(normalizeRole(e.target.value))} className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-800 focus:outline-none focus:ring-4 focus:ring-[#407B7E]/20">
                    <option value="reportee">Reportee</option>
                    <option value="qaOwner">QA Owner</option>
                    <option value="manager">Manager</option>
                  </select>
                ...
                <div className="flex items-end justify-end">
                  <button type="button" onClick={invite} disabled={inviting} className="h-10 px-4 rounded-xl bg-[#073D44] text-white font-semibold text-[13px] hover:bg-[#073D44]/90 disabled:opacity-60 transition-colors">
              ...
              <div>
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {projects.map(p => (
                    <label key={p.id} className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 bg-white">
                      <input type="checkbox" checked={inviteProjectIds.includes(p.id)} onChange={() => toggleInviteProject(p.id)} className="h-4 w-4 accent-[#073D44]" />
                      <span className="text-[13px] font-semibold text-slate-800 truncate">{p.code}</span>
                    </label>
                  ))}
                  {!projects.length && <div className="text-[13px] text-slate-500 font-semibold">No projects configured.</div>}
